...
public class IngressExtension extends MessageProcessor {
    ...
    public ModuleRegistrationResponse processRegistrationRequest(
        ...
        permissions.setUserIdentities(
                Arrays.asList(
                        new UserIdentityPermission(
                                UserIdentity.Type.EMAIL, Identity.Encoding.RAW, false),
                        new UserIdentityPermission(
                                UserIdentity.Type.CUSTOMER, Identity.Encoding.RAW, false)));
        permissions.setDeviceIdentities(
                Arrays.asList(
                        new DeviceIdentityPermission(
                                DeviceIdentity.Type.GOOGLE_CLOUD_MESSAGING_TOKEN,
                                Identity.Encoding.RAW),
                        new DeviceIdentityPermission(
                                DeviceIdentity.Type.APPLE_PUSH_NOTIFICATION_TOKEN,
                                Identity.Encoding.RAW),
                        new DeviceIdentityPermission(
                                DeviceIdentity.Type.IOS_VENDOR_ID, Identity.Encoding.RAW),
                        new DeviceIdentityPermission(
                                DeviceIdentity.Type.ANDROID_ID, Identity.Encoding.RAW),
                        new DeviceIdentityPermission(
                                DeviceIdentity.Type.GOOGLE_ADVERTISING_ID, Identity.Encoding.RAW)));
        ...
        response.setDescription(
                "<a href=\"https://www.iterable.com\">Iterable</a> makes consumer growth marketing and user engagement simple. With Iterable, marketers send the right message, to the right device, at the right time.");
        EventProcessingRegistration eventProcessingRegistration =
                new EventProcessingRegistration()
        ...
        Setting userIdField =
                new TextSetting(SETTING_USER_ID_FIELD, "User ID")
                        ...
                        .setDescription(
                                "Select which user identity to forward to Iterable as your customer's user ID.");
        ...
        eventSettings.add(
                new BooleanSetting(SETTING_COERCE_STRINGS_TO_SCALARS, "Coerce Strings to Scalars")
                        .setIsChecked(true)
                        .setDescription(
                                "If enabled, mParticle will attempt to coerce string attributes into scalar types (integer, boolean, and float)."));
        ...
        List<Event.Type> supportedEventTypes =
                Arrays.asList(
        ...
        listIdSetting.setDescription(
                "The ID of the Iterable list to populate with the users from this segment.");
