 ...
 *
 * <p>https://api.iterable.com/api/docs
 */
public interface IterableService {
    ...
    Call<IterableApiResponse> track(
            @Query(IterableService.PARAM_API_KEY) String apiKey, @Body TrackRequest trackRequest);

    @POST("api/events/trackPushOpen")
    Call<IterableApiResponse> trackPushOpen(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body TrackPushOpenRequest registerRequest);

    @POST("api/users/update")
    Call<IterableApiResponse> userUpdate(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body UserUpdateRequest trackRequest);

    @POST("api/users/updateEmail")
    Call<IterableApiResponse> updateEmail(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body UpdateEmailRequest updateEmailRequest);

    @POST("api/users/registerDeviceToken")
    Call<IterableApiResponse> registerToken(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body RegisterDeviceTokenRequest registerRequest);

    @POST("api/lists/subscribe")
    Call<ListResponse> listSubscribe(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body SubscribeRequest subscribeRequest);

    @POST("api/lists/unsubscribe")
    Call<ListResponse> listUnsubscribe(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body UnsubscribeRequest unsubscribeRequest);

    @POST("api/commerce/trackPurchase")
    Call<IterableApiResponse> trackPurchase(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body TrackPurchaseRequest purchaseRequest);

    @POST("api/users/updateSubscriptions")
    Call<IterableApiResponse> updateSubscriptions(
            @Query(IterableService.PARAM_API_KEY) String apiKey,
            @Body UpdateSubscriptionsRequest userUpdateRequest);
    ...
    class HeadersInterceptor implements Interceptor {
        ...
        public Response intercept(Interceptor.Chain chain) throws IOException {
            ...
            Request requestWithHeaders =
                    original.newBuilder().header("User-Agent", "mparticle-lambda").build();
    ...
    static IterableService newInstance() {
        final OkHttpClient client =
                new OkHttpClient.Builder()
                        ...
                        .build();
        final HttpUrl url =
                new HttpUrl.Builder().scheme("https").host(IterableService.HOST).build();
