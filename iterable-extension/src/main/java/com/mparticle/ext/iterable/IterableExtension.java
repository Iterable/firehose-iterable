...
public class IterableExtension extends MessageProcessor {
    ...
    @Override
    public ModuleRegistrationResponse processRegistrationRequest(
    ...
    private void processPushOpens(EventProcessingRequest processingRequest) throws IOException {
        ...
        if (processingRequest.getEvents() != null) {
            List<PushMessageOpenEvent> pushOpenEvents =
                    processingRequest
                            .getEvents()
                            .stream()
                ...
                if (event.getPayload() != null && processingRequest.getUserIdentities() != null) {
                    ...
                    if (request.email == null && request.userId == null) {
                        IterableExtensionLogger.logError(
                                "Unable to process PushMessageOpenEvent - user has no email or customer id.");
                        return;
                    }
                    ObjectMapper mapper = new ObjectMapper();
                    Map<String, Object> payload = mapper.readValue(event.getPayload(), Map.class);
                    if (payload.containsKey("itbl")) {
                        // Android and iOS have differently encoded payload formats. See the tests
                        // for examples.
                        Map<String, Object> iterableMap;
                        if (processingRequest.getRuntimeEnvironment()
                        ...
                        request.campaignId =
                                convertItblPayloadFieldToInt(iterableMap.get("campaignId"));
                        request.templateId =
                                convertItblPayloadFieldToInt(iterableMap.get("templateId"));
                        ...
                        Call<IterableApiResponse> preparedCall =
                                iterableService.trackPushOpen(
                                        getApiKey(processingRequest), request);
                        Response<IterableApiResponse> response =
    ...
    private void insertPlaceholderEmail(EventProcessingRequest request) throws IOException {
        long count =
                request.getUserIdentities() == null
                        ? 0
                        : request.getUserIdentities()
                                .stream()
        ...
        request.getUserIdentities()
                .add(
                        new UserIdentity(
                                UserIdentity.Type.EMAIL, Identity.Encoding.RAW, placeholderEmail));
    }

    /**
     * Map mParticle PushSubscriptionEvent to Iterable RegisterDeviceTokenRequest.
     *
     * <p>mParticle push_registration:
     * https://docs.mparticle.com/developers/server/json-reference/#push_registration Iterable
     * registerDeviceToken: https://api.iterable.com/api/docs#users_registerDeviceToken
     *
    ...
    public void processPushSubscriptionEvent(PushSubscriptionEvent event) throws IOException {
        ...
        if (event.getRequest()
                ...
                .getType()
                .equals(RuntimeEnvironment.Type.IOS)) {
            Boolean sandboxed =
                    ((IosRuntimeEnvironment) event.getRequest().getRuntimeEnvironment())
                            .getIsSandboxed();
            if (sandboxed != null && sandboxed) {
                request.device.platform = Device.PLATFORM_APNS_SANDBOX;
                request.device.applicationName =
                        event.getRequest()
                                .getAccount()
                                .getAccountSettings()
                                .get(SETTING_APNS_SANDBOX_KEY);
            } else {
                request.device.platform = Device.PLATFORM_APNS;
                request.device.applicationName =
                        event.getRequest().getAccount().getAccountSettings().get(SETTING_APNS_KEY);
            }
        } else if (event.getRequest()
                .getRuntimeEnvironment()
                .getType()
                .equals(RuntimeEnvironment.Type.ANDROID)) {
            request.device.platform = Device.PLATFORM_GCM;
            request.device.applicationName =
                    event.getRequest().getAccount().getAccountSettings().get(SETTING_GCM_NAME_KEY);
        } else {
            IterableExtensionLogger.logError(
                    "Cannot process push subscription event for unknown RuntimeEnvironment type.");
            return;
        }
        ...
        try {
            ...
            IterableExtensionLogger.logError(
                    "Unable to construct Iterable RegisterDeviceTokenRequest - no user email.");
            return;
        }

        Call<IterableApiResponse> preparedCall =
                iterableService.registerToken(getApiKey(event), request);
        Response<IterableApiResponse> response = makeIterableRequest(preparedCall, event.getId());
        handleIterableResponse(response, event.getId());
    }

    /**
     * Collect user attributes from mParticle request and make Iterable UpdateEmailRequest and/or
     * UserUpdateRequest.
     *
     * <p>mParticle user_attributes:
     * https://docs.mparticle.com/developers/server/json-reference/#user_attributes Iterable
     * updateUser: https://api.iterable.com/api/docs#users_updateUser Iterable updateEmail:
    ...
    void updateUser(EventProcessingRequest request) throws IOException {
        if (request.getEvents() != null) {
            ...
            List<UserIdentityChangeEvent> emailChangeEvents =
                    request.getEvents()
                            .stream()
                            ...
                            .filter(
                                    e ->
                                            e.getAdded()
                                                    .get(0)
                                                    .getType()
                                                    .equals(UserIdentity.Type.EMAIL))
                            .filter(
                                    e ->
                                            !isEmpty(e.getAdded().get(0).getValue())
                                                    && !isEmpty(e.getRemoved().get(0).getValue()))
                            .collect(Collectors.toList());

            List<UserIdentityChangeEvent> emailAddedEvents =
                    request.getEvents()
                            .stream()
                            .filter(e -> e.getType() == Event.Type.USER_IDENTITY_CHANGE)
                            .map(e -> (UserIdentityChangeEvent) e)
        ...
        if (request.getUserIdentities() != null) {
            ...
            if (!isEmpty(userUpdateRequest.email) || !isEmpty(userUpdateRequest.userId)) {
                userUpdateRequest.dataFields =
                        convertAttributes(
                                request.getUserAttributes(), shouldCoerceStrings(request));
                Call<IterableApiResponse> preparedCall =
    ...
    private Map<String, Object> convertAttributes(
            Map<String, String> attributes, boolean coerceStringsToScalars) {
        if (attributes == null) {
            return null;
        }
    ...
    private static boolean shouldCoerceStrings(EventProcessingRequest request) {
        String settingValue =
                request.getAccount().getAccountSettings().get(SETTING_COERCE_STRINGS_TO_SCALARS);
    ...
    private static boolean shouldUseMPID(Account account) {
        String settingValue =
                account.getStringSetting(SETTING_USER_ID_FIELD, false, USER_ID_FIELD_CUSTOMER_ID);
     ...
     *
     * <p>mParticle product_action:
     * https://docs.mparticle.com/developers/server/json-reference#product_action Iterable
     * trackPurchase: https://api.iterable.com/api/docs#commerce_trackPurchase
     *
    ...
    public void processProductActionEvent(ProductActionEvent event) throws IOException {
        if (event.getAction().equals(ProductActionEvent.Action.PURCHASE)) {
            ...
            apiUser.dataFields =
                    convertAttributes(event.getRequest().getUserAttributes(), shouldCoerceStrings);
            ...
            Call<IterableApiResponse> preparedCall =
                    iterableService.trackPurchase(getApiKey(event), purchaseRequest);
            Response<IterableApiResponse> response =
                    makeIterableRequest(preparedCall, event.getId());
            handleIterableResponse(response, event.getId());
        }
    ...
    /**
     * Make the best attempt at creating a placeholder email, prioritize: 1. MPID if enabled 2.
     * Platform respective device IDs 3. customer Id 4. device application stamps
     *
     * @param request
     * @return Also see:
    ...
    static String getPlaceholderEmail(EventProcessingRequest request) throws IOException {
        ...
        if (shouldUseMPID(request.getAccount())) {
            ...
            if (request.getRuntimeEnvironment() instanceof IosRuntimeEnvironment
                    || request.getRuntimeEnvironment() instanceof TVOSRuntimeEnvironment) {
                ...
                if (((AndroidRuntimeEnvironment) request.getRuntimeEnvironment()).getIdentities()
                        != null) {
                    DeviceIdentity deviceIdentity =
        ...
        if (isEmpty(id)) {
            ...
            throw new IOException(
                    "Unable to send user to Iterable - no email and unable to construct placeholder.");
        }
    ...
    /**
     * This is expected to be called with an event that conforms to the following: Name:
     * "updateSubscriptions"
     *
     * <p>And has at least some of the following:
     *
     * <p>Attribute: emailListIds Attribute: unsubscribedChannelIds Attribute:
     * unsubscribedMessageTypeIds Attribute: campaignId Attribute: templateId
     */
    private boolean processSubscribeEvent(CustomEvent event) throws IOException {
        ...
        Call<IterableApiResponse> preparedCall =
                iterableService.updateSubscriptions(getApiKey(event), updateRequest);
        Response<IterableApiResponse> response = makeIterableRequest(preparedCall, event.getId());
        handleIterableResponse(response, event.getId());
        return true;
    }
    ...
    static UpdateSubscriptionsRequest generateSubscriptionRequest(CustomEvent event) {
        ...
        updateRequest.unsubscribedChannelIds =
                convertToIntList(eventAttributes.get(UNSUBSCRIBE_CHANNEL_ID_LIST_KEY));
        updateRequest.unsubscribedMessageTypeIds =
                convertToIntList(eventAttributes.get(UNSUBSCRIBE_MESSAGE_TYPE_ID_LIST_KEY));
     ...
     *
     * <p>mParticle custom_event:
     * https://docs.mparticle.com/developers/server/json-reference/#custom_event Iterable track:
    ...
    /**
     * Make a best-effort attempt to coerce the values of each map item to bool, double, int, and
     * string types
     *
     * <p>mParticle's API only accepts string, whereas Iterable's API accept different types. By
     * coercing these types, users of the Iterable API are able to create campaigns, aggregate
     * events, etc.
     *
     * @param attributes
     * @return
                ...
                (key, value) -> {
                    if (isEmpty(value)) {
                        ...
                        if (value.toLowerCase(Locale.US).equals("true")
                                || value.toLowerCase(Locale.US).equals("false")) {
     ...
     *
     * <p>mParticle push_message event
     * https://docs.mparticle.com/developers/server/json-reference#push_message Iterable
     * trackPushOpen request: https://api.iterable.com/api/docs#events_trackPushOpen
     *
     * @param event the mParticle event
     * @throws IOException
     */
    ...
    public void processPushMessageReceiptEvent(PushMessageReceiptEvent event) throws IOException {
        ...
        if (event.getPayload() != null && event.getRequest().getUserIdentities() != null) {
            ...
            if (request.email == null && request.userId == null) {
                IterableExtensionLogger.logError(
                        "Unable to process PushMessageReceiptEvent - user has no email or customer id.");
                return;
            }
            ObjectMapper mapper = new ObjectMapper();
            Map<String, Object> payload = mapper.readValue(event.getPayload(), Map.class);
            if (payload.containsKey("itbl")) {
                // Android and iOS have differently encoded payload formats. See the tests for
                // examples.
                Map<String, Object> iterableMap;
                ...
                request.createdAt = (int) (event.getTimestamp() / 1000.0);
                Call<IterableApiResponse> preparedCall =
                        iterableService.trackPushOpen(getApiKey(event), request);
                Response<IterableApiResponse> response =
                        makeIterableRequest(preparedCall, event.getId());
                handleIterableResponse(response, event.getId());
            }
    ...
    /**
     * Map mParticle AudienceMembershipChangeRequest to Iterable SubscribeRequests and
     * UnsubscribeRequests.
     *
     * <p>Each subscribe and unsubscribe request may contain multiple users if there are multiple
     * users being added or removed from the same list. No dataFields are sent with the users.
     *
     * <p>mParticle Audience Processing:
     * https://docs.mparticle.com/developers/partners/firehose/#audience-processing Iterable
     * subscribe: https://api.iterable.com/api/docs#lists_subscribe Iterable unsubscribe:
    ...
    public AudienceMembershipChangeResponse processAudienceMembershipChangeRequest(
        ...
        for (UserProfile profile : request.getUserProfiles()) {
            ...
            if (userRequest.email != null) {
                if (profile.getAudiences() != null) {
                    List<Audience> addedAudiences =
                            profile.getAudiences()
                                    .stream()
                                    .filter(
                                            audience ->
                                                    audience.getAudienceAction()
                                                            == Audience.AudienceAction.ADD)
                                    .collect(Collectors.toList());
                    List<Audience> removedAudiences =
                            profile.getAudiences()
                                    .stream()
                                    .filter(
                                            audience ->
                                                    audience.getAudienceAction()
                                                            == Audience.AudienceAction.DELETE)
                                    .collect(Collectors.toList());
                    for (Audience audience : addedAudiences) {
                        Map<String, String> audienceSettings =
                                audience.getAudienceSubscriptionSettings();
        ...
        for (Map.Entry<Integer, List<ApiUser>> entry : additions.entrySet()) {
            ...
            Call<ListResponse> preparedCall =
                    iterableService.listSubscribe(getApiKey(request), subscribeRequest);
        ...
        for (Map.Entry<Integer, List<ApiUser>> entry : removals.entrySet()) {
            ...
            Call<ListResponse> preparedCall =
                    iterableService.listUnsubscribe(getApiKey(request), unsubscribeRequest);
            Response<ListResponse> response = makeIterableRequest(preparedCall, request.getId());
            handleIterableListResponse(response, request.getId());
        }
    ...
    private void addUserIdentitiesToRequest(
            UserRequest request, EventProcessingRequest processingRequest) {
        addUserIdentitiesToRequest(
                request,
                processingRequest.getUserIdentities(),
                processingRequest.getAccount(),
                processingRequest.getMpId());
    }

    private void addUserIdentitiesToRequest(
            UserRequest request, UserProfile userProfile, Account account) {
        addUserIdentitiesToRequest(
                request, userProfile.getUserIdentities(), account, userProfile.getMpId());
    }

    private void addUserIdentitiesToRequest(
            UserRequest request, List<UserIdentity> identities, Account account, String mpid) {
        if (identities != null) {
            for (UserIdentity identity : identities) {
                if (identity.getType().equals(UserIdentity.Type.EMAIL)) {
                    request.email = identity.getValue();
                } else if (identity.getType().equals(UserIdentity.Type.CUSTOMER)
                        && !shouldUseMPID(account)) {
    ...
    static void handleIterableListResponse(Response<ListResponse> response, UUID audienceRequestId)
        ...
        if (hasFailures) {
            IterableExtensionLogger.logError(
                    "List subscribe or unsubscribe request failed count: "
                            + response.body().failCount);
        }
    }
